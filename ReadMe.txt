================================================================================
    MFC 라이브러리 : ImageProcessingProject 프로젝트 개요
================================================================================

응용 프로그램 마법사에서 이 ImageProcessingProject 응용 프로그램을 
만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 
프로그램작성을 위한 기본 구조를 제공합니다.

이 파일에는 ImageProcessingProject 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.

ImageProcessingProject.vcxproj
...
ImageProcessingProject.vcxproj.filters
...
ImageProcessingProject.h
    응용 프로그램의 기본 헤더 파일입니다. 여기에는 다른 프로젝트 관련
    헤더(Resource.h 포함)가 들어 있고 CImageProcessingProjectApp 응용 프로그램
    클래스를 선언합니다.

ImageProcessingProject.cpp
    응용 프로그램 클래스 CImageProcessingProjectApp이(가) 들어 있는 기본 응용 프로그램
    소스 파일입니다.

ImageProcessingProject.rc
...
res\ImageProcessingProject.ico
    응용 프로그램의 아이콘으로 사용되는 아이콘 파일입니다. 이 아이콘은
    주 리소스 파일인 ImageProcessingProject.rc에 의해 포함됩니다.

res\ImageProcessingProject.rc2
    이 파일에는 Microsoft Visual C++ 이외의 다른 도구에서 편집한 리소스가 
    들어 있습니다. 리소스 편집기로 편집할 수 없는 모든 리소스는
    이 파일에 넣어야 합니다.

ImageProcessingProject.reg
...
ImageProcessingProjectDoc.h, ImageProcessingProjectDoc.cpp - 문서
    이 파일에는 CImageProcessingProjectDoc 클래스가 들어 있습니다. 이 파일을 편집하여
    특수한 문서 데이터를 추가하고 CImageProcessingProjectDoc::Serialize를 통해 파일
    저장 및 로드를 구현합니다.
    이 문서에는 다음 문자열이 포함됩니다.
        파일 확장명:      raw
        파일 형식 ID:        ImageProcessingProject.Document
        주 프레임 캡션:  ImageProcessingProject
        문서 형식 이름:       ImageProcessing
        필터 이름:         ImageProcessingProject Files (*.raw)
        파일의 새 약식 이름: ImageProcessing
        파일 형식의 긴 이름: ImageProcessingProject.Document

ImageProcessingProjectView.h, ImageProcessingProjectView.cpp - 문서 뷰
    이 파일에는 CImageProcessingProjectView 클래스가 들어 있습니다.
    CImageProcessingProjectView 개체는 CImageProcessingProjectDoc 개체를 보는 데 사용됩니다.

res\ImageProcessingProjectDoc.ico
    CImageProcessingProjectDoc 클래스의 MDI 자식 창에 대한 아이콘으로 사용되는
    아이콘 파일입니다. 이 아이콘은 주 리소스 파일인 ImageProcessingProject.rc에
    의해 포함됩니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더 파일(PCH)인 ImageProcessingProject.pch와
...
ImageProcessingProject.manifest
